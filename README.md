...
- When GitHub is unreachable, the CMS falls back to the cached snapshot for the current branch and quietly stays in offline mode until the user clicks `Refresh`. Offline mode blocks editing, branch switching, PR creation, Actions refreshes, and other GitHub API calls.
