...
- Global search excludes the same hidden root-level technical files and folders that are omitted from the tree, while still allowing `AGENTS.md`.
- `AGENTS.md` is rendered as a muted technical file in the tree, including at the root level.
- Markdown front matter titles in the tree when available, hiding the filename when a title exists on both files and folders.
- Tree files hide size values while directory rows keep file counts.
- Static asset cache-busting is shared across `index.html` and the JS module import chain so CSS and JS changes load together.
...
Decision: Markdown files and folders can display their front matter `title` in the tree, hiding the filename when a title is available. The original capped background scan was superseded on 2026-06-08 by startup content hydration and blob-SHA caching.
...
Decision: Tree file rows no longer display file sizes, while directory rows keep their file counts.
